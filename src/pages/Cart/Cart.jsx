...
function Cart () {
  ...
  cart.map((item) => (
    <div
      key={item.id}
      className="w-full max-w-2xl bg-white flex flex-col sm:flex-row items-center shadow-md gap-4 sm:gap-6 rounded-xl p-4 sm:p-5"
    >
      <div>
        <img
          src={item.img}
      ...
      <div className="flex-1 text-center sm:text-left">
        ...
        <p className="px-3 py-2 text-lg sm:text-xl">₹{item.price}</p>
        ...
        <div className="flex items-center justify-center sm:justify-start h-10 w-28 border rounded p-2 mx-auto sm:mx-0">
          ...
          <span className="text-lg sm:text-xl">{item.qty}</span>
          <button
            onClick={() => updateQty(item.id, item.qty + 1)}
            className="px-3 py-1 text-lg sm:text-xl"
          >
      ...
      <button
        ...
        className="text-red-500 font-semibold mt-2 sm:mt-0 sm:self-end"
      >
        Remove
      </button>
    </div>
